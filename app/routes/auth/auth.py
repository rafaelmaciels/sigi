...
from utils.seed import seed_permissions # 👈 importa o seed
...
def setup():
    ...
    if form.validate_on_submit():
        ...
        # 🔹 Popula permissões automaticamente se ainda não existirem
        seed_permissions()
    ...
    if form.validate_on_submit():
        ...
        if user and user.check_password(form.senha.data):
            if not getattr(user, "ativo", True):
                ...
                registrar_log(user.nome or "desconhecido", "Tentativa de login com usuário desativado", "erro")
                return redirect(url_for('auth.login'))

            login_user(user, remember=True)
            registrar_log(user.nome or "desconhecido", "Login realizado", "sucesso")
            ...
            if user:
                registrar_log(user.nome or "desconhecido", "Tentativa de login inválida (senha incorreta)", "erro")
            else:
                registrar_log(form.email.data.lower() or "não informado", "Tentativa de login inválida (usuário inexistente)", "erro")
...
def logout():
    if hasattr(current_app, "login_manager") and current_app.login_manager._login_disabled is False:
        if current_user.is_authenticated:
            registrar_log(current_user.nome or "desconhecido", "Logout realizado", "sucesso")
    logout_user()
...
def forgot_password():
    ...
    if form.validate_on_submit():
        ...
        if user:
            ...
            try:
                mail.send(msg)
                registrar_log(user.nome or "desconhecido", "Solicitou redefinição de senha", "sucesso")
            except Exception as e:
                current_app.logger.error(f"Erro ao enviar e-mail: {e}")
                registrar_log(user.nome or "desconhecido", "Erro ao enviar e-mail de redefinição", "erro")
    ...
    return render_template('auth/forgot_password.html', form=form, hide_navbar=True, hide_footer=True)
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        registrar_log(user.nome or "desconhecido", "Senha redefinida", "sucesso")
